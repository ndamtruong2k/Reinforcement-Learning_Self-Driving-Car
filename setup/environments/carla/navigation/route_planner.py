...
from setup.environments.carla.navigation.global_route_planner_dao import GlobalRoutePlannerDAO
from setup.environments.carla.navigation.road_option import RoadOption
from setup.environments.carla.tools.utils import unit_vector


class RoutePlanner(object):
    """Provides a high-level route plan."""

    def __init__(self, map, sampling_resolution=1):
        self._dao = GlobalRoutePlannerDAO(map, sampling_resolution)
        self._topology = None
        self._graph = None
        self._id_map = None
    ...
    def setup(self):
        ...
        and builds graph representation of the worlds map.
        """
    ...
    def _build_graph(self):
        ...
        graph node properties:
            vertex   -   (x,y,z) position in worlds map
        graph edge properties:
            ...
            intersection    -   boolean indicating if the edge belongs to an
                                intersection
        return      :   graph -> networkx graph representing the worlds map,
        ...
        for segment in self._topology:
            ...
            graph.add_edge(
                ...
                net_vector=unit_vector(entry_wp.transform.location, exit_wp.transform.location),
                intersection=intersection,
                type=RoadOption.LANE_FOLLOW)
            ...
            if road_id in self._road_id_to_edge and \
                ...
                if path:
                    ...
                    self._graph.add_edge(
                        ...
                        intersection=end_wp.is_intersection, type=RoadOption.LANE_FOLLOW)
        ...
        if index > 0:
            if self._previous_decision != RoadOption.VOID and \
                ...
                self._intersection_end_node != previous_node and \
                    next_edge['type'] == RoadOption.LANE_FOLLOW and \
                ...
                calculate_turn = current_edge['type'].value == RoadOption.LANE_FOLLOW.value and \
                    not current_edge['intersection'] and \
                    next_edge['type'].value == RoadOption.LANE_FOLLOW.value and \
                    next_edge['intersection']
                ...
                if calculate_turn:
                    ...
                    for neighbor in self._graph.successors(current_node):
                        ...
                        if select_edge['type'].value == RoadOption.LANE_FOLLOW.value:
    ...
    def abstract_route_plan(self, origin, destination):
        ...
        worlds.navigation.local_planner.RoadOption elements
    ...
    def trace_route(self, origin, destination):
        ...
        for i in range(len(route) - 1):
            ...
            if edge['type'].value != RoadOption.LANE_FOLLOW.value and edge['type'].value != RoadOption.VOID.value:
